/**
 * api/status.js — Health checks de geoservicios
 *
 * GET /api/status?health=ign_ar    → health check de una fuente específica
 * GET /api/status?health=all       → health check de todas las fuentes
 *
 * Extraído de api/config.js para separar configuración pública (estable,
 * cacheable) de estado operacional (dinámico, sin caché).
 *
 * Esta función está preparada para recibir también parámetros de
 * estadísticas de uso en el futuro (ej: ?stats=true), consolidando
 * en un único endpoint toda la información que necesita /status.
 */
...
module.exports = async function handler(req, res) {
  ...
  if (!health) {
    return res.status(400).json({
      error:     'Se requiere parámetro health',
